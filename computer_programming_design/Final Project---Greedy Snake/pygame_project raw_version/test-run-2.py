...
#架設狀態的計時器
upposite_direction_b=pygame.USEREVENT+1
upposite_direction_control_b =0
pygame.time.set_timer(upposite_direction_b,0)

speed_up_a=pygame.USEREVENT + 2
speed_up_control_a =0
pygame.time.set_timer(speed_up_a,0)

score_up_a=pygame.USEREVENT + 3
score_up_control_a =0
pygame.time.set_timer(score_up_a,0)

fixed_direction_b= pygame.USEREVENT + 4
fixed_direction_control_b =0
pygame.time.set_timer(fixed_direction_b,0)

upposite_direction_a= pygame.USEREVENT + 5
upposite_direction_control_a =0
pygame.time.set_timer(upposite_direction_a,0)

speed_up_b= pygame.USEREVENT + 6
speed_up_control_b =0
pygame.time.set_timer(speed_up_b,0)

score_up_b=pygame.USEREVENT + 7
score_up_control_b=0
pygame.time.set_timer(score_up_b,0)

fixed_direction_a= pygame.USEREVENT + 8
fixed_direction_control_a =0
pygame.time.set_timer(fixed_direction_a,0)

#時間限制的計時器
time_limit=pygame.USEREVENT + 9
pygame.time.set_timer(time_limit,1000)
time_count=0

#cool dowm的計時器
cool_down_a=pygame.USEREVENT + 10
pygame.time.set_timer(cool_down_a,0)
cool_down_b=pygame.USEREVENT + 11
...
#以下10個都是前面設置的計時器，前8個為兩條蛇碰到道具後開啟(snake_a,snake_b各四個),後兩者為兩條蛇條蛇snake_halfed之後開啟
                
        elif event.type == upposite_direction_a:             #狀態a_upposite_direction的計時器
            if upposite_direction_control_a==0:              #control是在下面的程式碼，碰到道具時給予，決定狀態持續的秒數。contrl==0意味著該狀況應該要結束了
                pygame.time.set_timer(upposite_direction_a,0)#結束a_upposite_direction狀態的計時器
                a_upposite_direction=False                   #結束狀態a_upposite_direction
            else:
                a_upposite_direction=True                    #維持狀態a_upposite_direction
                upposite_direction_control_a-=1              #縮短狀態a_upposite_direction會維持的時間(1 control=1秒)
        elif event.type == speed_up_a:
        ...
        elif event.type ==fixed_direction_a:
        ...
        elif event.type ==upposite_direction_b:
        ...
        elif event.type == fixed_direction_b:
...
# snake a moving logic，只對snake_a寫註解，因為snake_b格式也一樣
    ...
    #碰到道具的時候，註解只寫第一項，因為格式都一樣
        if snake_head_a in tool_upposite_direction:
            upposite_direction_control_b+=5                 #決定秒數
            pygame.time.set_timer(upposite_direction_b,1000)#啟動計時器
            tool_upposite_direction.remove(snake_head_a)    #在螢幕上消解該道具
            b_upposite_direction=True                       #啟動狀況
        if snake_head_a in tool_speed_up:
            speed_up_control_a+=10
            pygame.time.set_timer(speed_up_a,1000)
            tool_speed_up.remove(snake_head_a)
            a_speed_up=True
        if snake_head_a in tool_score_up:
            score_up_control_a+=30
            pygame.time.set_timer(score_up_a,1000)
            tool_score_up.remove(snake_head_a)
            a_score_up=True
        if snake_head_a in tool_fixed_direction:
            fixed_direction_control_b+=2
            pygame.time.set_timer(fixed_direction_b,1000)
    ...
    for i in range(move_time_b):
        ...
        if snake_head_b in tool_upposite_direction:
            ...
            pygame.time.set_timer(upposite_direction_a,1000)
